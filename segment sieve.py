...
# fillPrime functions filles primes from 2 to sqrt to high in chprime 11.
...
# in segment sieve we check for prime in range[low, high]
...
# here prime[0] indicates whether low is prime or not similarily prime.
    for i in chprime:
        lower = (low//i)
# here lower means the first multiple of prime which is in range [low, high]
# eg:- 3's first multiple in range [28,40] is 30
        if lower <= 1:
            lower = i+i
        elif (low % i) !=0:
            lower = (lower * i) + i
...
print("Primes in Range %d to %d "%(low,high))
